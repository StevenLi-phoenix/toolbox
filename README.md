...
| Tool | Description |
|------|-------------|
| [Apdex Score Calculator](apdex.html) | Compute an Apdex (Application Performance Index) score from a latency target `T`. Enter satisfied / tolerating / frustrated counts, or paste raw latencies and let the tool bin them into the `≤ T`, `T .. 4T`, and `> 4T` zones. Returns the score with a verdict band, per-zone counts and shares, and a small histogram. |
| [MTBF & Availability Calculator](mtbf.html) | Compute MTBF, MTTR, and inherent availability (`A = MTBF / (MTBF + MTTR)`) from a list of incident durations — or start from an availability SLO (percent or "nines") and get the allowed downtime per hour, day, month, and year, plus the implied MTBF for a given MTTR. Includes a nines reference table. |
...
| Tool | Description |
|------|-------------|
| [Elo Rating Calculator](elo.html) | Compute the expected score and K-factor rating change after a chess game — supports FIDE's auto K schedule (40 for new players, 20 established, 10 once 2400+), the common online K=32 default, or any custom K. Batch mode ingests a list of `opponent,result` rows and shows a per-game breakdown; single-game mode plots the sensitivity of Δ against actual score. |
